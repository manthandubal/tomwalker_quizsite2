...
class QuizUserProgressView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['incomplete_exams'] = progress.show_incomplete_exams()
        return context
...
class QuestionList(APIView):
    def get(self, request, quiz_name):
        question = Question.objects.filter(quiz__url = quiz_name).order_by('id')
...
class SubCategoryList(APIView):
    def get(self, request, category_name):
        sub_category = SubCategory.objects.filter(category__category = category_name).order_by('sub_category')
        serializer = SubCategorySerializer(sub_category, many= True)
        return Response(serializer.data)

class ProgressList(APIView):
    def get(self, request):   
        # In v 1.11 if self.request.user.is_authenticated():    
        if self.request.user.is_authenticated:
            progress = Progress.objects.all()
            serializer = ProgressSerializer(progress, many= True)
            return Response(serializer.data)
        else:
           return HttpResponse("You must be logged in get the progress")

class MarkingList(APIView):
    def get(self, request):    
        # In v 1.11 if self.request.user.is_authenticated():
        if self.request.user.is_authenticated:
            marking = Sitting.objects.all()
            serializer = MarkingSerializer(marking, many= True)
            return Response(serializer.data)
        else:
           return HttpResponse("You must be logged in get the progress")
...
"""
Continue taking Quiz from place it left.
"""

class ContinueQuizTake(FormView):
    form_class = QuestionForm
    template_name = 'question.html'
    result_template_name = 'result.html'
    single_complete_template_name = 'single_complete.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        # print("kwargs quizname - ", self.kwargs['quiz_name'])
        # print("2. kwargs quizname - ", self.kwargs['quiz_name'], " type of ", type(self.kwargs['pk']))
        self.quiz = get_object_or_404(Quiz, url=self.kwargs['quiz_name'])
        if self.quiz.draft and not request.user.has_perm('quiz.change_quiz'):
            raise PermissionDenied
        ...
        if self.logged_in_user:
            self.sitting = Sitting.objects.filter(id = int(self.kwargs['pk'])).first()
            #print(sitting__question_order)
        else:
        ...
        return super(ContinueQuizTake, self).dispatch(request, *args, **kwargs)
    ...
    def get_form_kwargs(self):
        kwargs = super(ContinueQuizTake, self).get_form_kwargs()
    ...
    def form_valid(self, form):
        ...
        return super(ContinueQuizTake, self).get(self, self.request)

    def get_context_data(self, **kwargs):
        context = super(ContinueQuizTake, self).get_context_data(**kwargs)
        context['question'] = self.question
        context['quiz'] = self.quiz
        if hasattr(self, 'previous'):
            context['previous'] = self.previous
